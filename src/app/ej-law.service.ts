...
export class EjLawService {
  ...
  tagWebLinks(doc) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].innerHTML.match(/(http.*?)(\s|$)/)) {
        lines[i].innerHTML = lines[i].innerHTML.replace(/(http\s|http)(.*?)(\s|$)/, `<a href="http$2">http$2</a> `);
      }
    }
    return doc;
  }
  ...
  tagParagraphs(doc) {
    ...
    for (let i = 0; i < articles.length; i++) {
      let counter = 1;
      const lines = articles[i].innerHTML.split('<br>&nbsp;&nbsp;');
      ...
      articles[i].innerHTML = lines.join('<br>');
    }
    return doc;
  }
  ...
  tagArtRef(doc) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i]) {
        lines[i].innerHTML = lines[i].innerHTML.replace(/(Art(\.|ikel)\s\d{1,3}[a-z]*)/gi, '<artref>$1</artref>');
      }
    }
    return doc;
  }
